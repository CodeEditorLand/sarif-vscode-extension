...
import { type KeyboardEvent, PureComponent, type ReactNode, memo } from "react";
import { Badge, Hi, Icon, ResizeHandle, css } from "./widgets";
import "./table.scss";
import { type Column, RowGroup, RowItem, type TableStore } from "./tableStore";
...
@observer
export class Table<T, G> extends PureComponent<TableProps<T, G>> {
	@computed get gridTemplateColumns() {
		...
		return [
			...
			...columns.map(
				(col, i) => `${(i === 0 ? 22 : 0) + col.width.get()}px`,
			),
			"1fr", // Fill remaining space so the the selection/hover highlight doesn't look funny.
		].join(" ");
	}

	private TableItem = memo<{
		isLineThrough: boolean;
		isSelected: boolean;
		item: RowItem<T>;
		gridTemplateColumns: string;
		menuContext: Record<string, string> | undefined;
	}>((props) => {
			...
			<div
				...
				{columns.map((col, i) => (
					<Hi
						...
						{i === 0 && renderIconName && (
							<Icon name={renderIconName(item.item)} />
						)}
				...
				<div
					...
					{columns.map((col) => (
						<div
							...
							{col.name}
							{/* No spacing */}
							{store.sortColumn === col.name && (
								<Icon title="Sort" name={store.sortDir} />
							)}
							<ResizeHandle size={col.width} horizontal={true} />
						</div>
					))}
				</div>
				<div
					tabIndex={0}
					class={css("svTableBody", selection.get() && "svSelected")}
					...
					{rows.map((row) => {
						...
						if (row instanceof RowItem) {
							...
							return (
								<TableItem
									key={row.key}
									isLineThrough={store.isLineThrough(
										row.item,
									)}
	...
	@action.bound private onKeyDown(e: KeyboardEvent<Element>) {
		...
		const handlers = {
			ArrowUp: () =>
				selection.set(rows[index - 1] ?? rows[index] ?? rows[0]),
