...
import type { ReportingDescriptor } from "sarif";
...
import { type IndexStore, postLoad, postRefresh } from "./indexStore";
...
@observer
export class Index extends Component<{ store: IndexStore }> {
	...
	render() {
		...
		const bannerElement = banner && (
			<div class="svBanner">
				...
				<div class="svButton" onClick={() => postRefresh()}>
					Refresh results
				</div>
			</div>
		);
		...
		return (
			<FilterKeywordContext.Provider value={keywords ?? ""}>
				...
				<div class="svListPane">
					<TabPanel
						...
						extras={
							<>
								...
								<Icon
									...
									onClick={() =>
										activeTableStore?.groupsFilteredSorted.forEach(
											(group) =>
												(group.expanded = allCollapsed),
										)
									}
								/>
								<Icon
						...
						<Tab
							name={store.tabs[0]}
							count={
								store.resultTableStoreByLocation
									.groupsFilteredSorted.length
							}>
							<ResultTable
								store={store.resultTableStoreByLocation}
								...
								renderGroup={(title: string) => {
									...
									return (
										<>
											<span>
												{pathname.file || "No Location"}
											</span>
											<span class="ellipsis svSecondary">
												{pathname.path}
											</span>
										</>
									);
						...
						<Tab
							name={store.tabs[1]}
							count={
								store.resultTableStoreByRule
									.groupsFilteredSorted.length
							}>
							<ResultTable
								store={store.resultTableStoreByRule}
								onClearFilters={() => store.clearFilters()}
								renderGroup={(
						...
						<Tab name={store.tabs[2]} count={logs.length}>
							<div class="svLogsPane">
								{logs.map((log, i) => {
									const { pathname } = new URL(log._uri);
									return (
										<div key={i} class="svListItem">
											<div>{pathname.file}</div>
											<div class="ellipsis svSecondary">
				...
				<Details
					result={selected}
					resultsFixed={store.resultsFixed}
					height={detailsPaneHeight}
				/>
				<Popover
					show={showFilterPopup}
					style={{ top: 35, right: 8 + 35 + 35 + 8 }}>
					{Object.entries(store.filtersRow).map(([name, state]) => (
						<Fragment key={name}>
							<div class="svPopoverTitle">{name}</div>
					...
					<div class="svPopoverDivider" />
					{Object.entries(store.filtersColumn).map(
						([name, state]) => (
							<Fragment key={name}>
								<div class="svPopoverTitle">{name}</div>
								{Object.keys(state).map((name) => (
