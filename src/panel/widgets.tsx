...
import { type IObservableValue, action } from "mobx";
...
export class Badge extends PureComponent<{ text: { toString: () => string } }> {
	render() {
		return <span class="svBadge">{this.props.text.toString()}</span>;
	}
}

@observer
export class Checkrow extends PureComponent<{
	label: string;
	description?: string;
	state: Record<string, Visibility>;
}> {
	render() {
		const { label, description, state } = this.props;
		return (
			<div
				class={css("svCheckrow", description && "svWithDescription")}
				onClick={() =>
					(state[label] =
						state[label] === "visible" ? false : "visible")
				}>
				<div
					class={css("svCheckbox", state[label] && "svChecked")}
					tabIndex={0}
					role="checkbox"
					aria-checked="false"
					aria-label=""
...
> {
	render() {
		...
		return <div class={`codicon codicon-${name}`} {...divProps}></div>;
	}
}
...
export class Hi extends React.Component<React.HTMLAttributes<HTMLDivElement>> {
	...
	render() {
		let term = this.context as React.ContextType<
			typeof FilterKeywordContext
		>;
		function hi(children: React.ReactNode): React.ReactNode {
			...
			return (children + "")
				...
				.map((word, i) =>
					i % 2 === 1 ? <mark key={i}>{word}</mark> : word,
				);
		}
	...
	render() {
		...
		return items?.length ? (
			<div
				...
				class={css(
					"svList",
					selection.get() && "svSelected",
					className,
				)}
				onClick={() => allowClear && selection.set(undefined)}
			...
			<div class={css("svList", "svListZero", className)}>{children}</div>
		);
	}
...
export class Popover extends PureComponent<
	{ show: IObservableValue<boolean> } & React.HTMLAttributes<HTMLDivElement>
> {
	render() {
...
class OptionalDiv extends Component<React.HTMLAttributes<HTMLDivElement>> {
	render() {
		const { children, ...divProps } = this.props;
		const nonEmptyChildren = React.Children.toArray(children).filter(
			(c) => c,
		);
...
@observer
export class TabPanel<T> extends PureComponent<{
	...
	render() {
		...
		return (
			<>
				<OptionalDiv class="svListHeader">
					{/* Abstraction break: svListHeader */}
					<List
						class="svTabs"
						horizontal={true}
						items={tabNames}
...
export class ResizeHandle extends Component<{
	...
	@action.bound private onMouseMove(e: MouseEvent) {
		...
		this.props.size.set(
			Math.max(0, this.startingSize + delta * (horizontal ? -1 : 1)),
		);
	...
	render() {
		...
		return (
			<div
				...
				onClick={
					(e) =>
						e.stopPropagation() /* Prevent click in situations such as triggering a table header cell sort. */
				}></div>
		);
	}
}
...
export function renderMessageTextWithEmbeddedLinks(
	text: string,
	result: Result,
	_postMessage: (_: unknown) => void,
) {
	if (text) {
