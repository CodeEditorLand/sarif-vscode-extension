...
import { type IArrayWillSplice, computed, intercept, observable } from "mobx";
import type { Log } from "sarif";
import type { Memento } from "vscode";
import { mapDistinct } from "../shared";
import "../shared/extension";
import type { AnalysisInfosForCommit } from "./index.activateGithubAnalyses";
...
export class Store {
	...
	@computed get results() {
		const runs = this.logs.flatMap((log) => log.runs);
		return runs
			.flatMap((run) => run.results ?? [])
			.filter(
				(result) =>
					!this.resultsFixed.includes(JSON.stringify(result._id)),
			);
	}
	@computed get distinctArtifactNames() {
		const fileAndUris = this.logs.flatMap((log) => [
