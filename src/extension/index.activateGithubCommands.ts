...
import { type ResultId, findResult } from "../shared";
...
export function activateGithubCommands(
	...
	async function dismissAlert(
		...
		const response = await callGithubRepos(
			`${ownerAndRepo}/alerts/${alertNumber}`,
			{
				state: "dismissed",
	...
	disposables.push(
		commands.registerCommand(
			"sarif.alertDismissFalsePositive",
			async (context) => dismissAlert(context, "false positive"),
		),
		commands.registerCommand(
			"sarif.alertDismissUsedInTests",
			async (context) => dismissAlert(context, "used in tests"),
		),
		commands.registerCommand("sarif.alertDismissWontFix", async (context) =>
			dismissAlert(context, "won't fix"),
		),
	);
}

// `api` does not include leading slash.
async function callGithubRepos(
	api: string,
