...
import type { Log, ReportingDescriptor } from "sarif";
...
export async function loadLogs(
	...
	const warnUpgradeExtension = logs.some((log) =>
		detectSupport(log, logsSupported, logsNotSupported),
	);
	for (const log of logsNotSupported) {
		if (token?.isCancellationRequested) break;
		const { fsPath } = Uri.parse(log._uri, true);
		window.showWarningMessage(
			`'${fsPath}' was not loaded. Version '${log.version}' and schema '${
				log.$schema ?? ""
			}' is not supported.`,
		);
	}
