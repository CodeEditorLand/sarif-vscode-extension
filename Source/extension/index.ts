...
export async function activate(context: ExtensionContext): Promise<Api> {
	...
	const isDebugOrTestMode =
		...
		process.execArgv.some((arg) =>
			/^--(debug|debug-brk|inspect|inspect-brk)=?/.test(arg),
		); // Test
				...
				if (uri.path.toLowerCase() === "/alert") {
					...
					if (url.startsWith("https://advsec.dev.azure.com/")) {
						...
						void window.showWarningMessage(
							`Invalid callback URL '${url}'. URL must point to 'https://advsec.dev.azure.com/'.`,
							"OK",
						);
					}
				}
			...
			if (response.ok) {
				...
				try {
					...
					outputChannel.appendLine(
						`***Exception in loadAlertSarif\n***${error}\n***File path: ${filePath}\n`,
					);
				}
			} else {
				outputChannel.appendLine(
					`***Failed request in loadAlertSarif\n***Response code ${
						response.status
					} ${response.statusText}\n***URL: ${url.toString()}\n`,
				);
			}
	...
	const api = {
		...
		get uriBases() {
			return baser.uriBases.map((uri) =>
				Uri.file(uri),
			) as ReadonlyArray<Uri>;
		},
		set uriBases(values) {
...
function activateDiagnostics(
	...
	const setDiags = async (doc: TextDocument) => {
		...
		outputChannel.appendLine(
			`updateDiags ${doc.uri.toString().replace(workspaceUri, "")}. ${
		...
		const diags = matchingResults.map((result) => {
			return new ResultDiagnostic(
				...
				result._message ?? "—",
				severities[result.level ?? ""] ??
					DiagnosticSeverity.Information, // note, none, undefined.
				result,
			);
		});
	...
	disposables.push(
		workspace.onDidChangeTextDocument(({ document }) => setDiags(document)),
	); // TODO: Consider updating the regions independently of the list of diagnostics.
...
function activateWatchDocuments(
	disposables: Disposable[],
	store: Store,
	panel: Panel,
) {
				...
				if (contents?.binary) {
					...
					return (
						lines?.reduce((sum, line, i) => {
							...
							const preview = Buffer.from(line, "hex")
								.toString("utf8")
								.replace(
...
function activateSelectionSync(
	...
	disposables.push(
		window.onDidChangeTextEditorSelection(({ selections, textEditor }) => {
