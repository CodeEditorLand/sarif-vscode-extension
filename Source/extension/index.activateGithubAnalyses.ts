...
import type { Fix, Log, Result } from "sarif";
import {
	ConfigurationTarget,
	type Disposable,
	type OutputChannel,
...
export function getPrimaryRepository(git: API): Repository | undefined {
	const primaryWorkspaceFolderUriString =
		workspace.workspaceFolders?.[0]?.uri.toString(); // No trailing slash
	return git.repositories.filter(
		(repo) => repo.rootUri.toString() === primaryWorkspaceFolderUriString,
	)[0];
}
...
export function activateGithubAnalyses(
	...
	(async () => {
		...
		if (connectToGithubCodeScanning === "prompt") {
			...
			if (choice === "Never") {
				...
				const analysisFound = await window.withProgress<boolean>(
	...
	async function fetchAnalysisInfo(
	...
	): Promise<AnalysisInfosForCommit | undefined> {
		...
		return {
			...
			commitsAgo: commits.findIndex(
				(commit) => commit.hash === intersectingCommit,
			),
		};
	}

	function updateAnalysisInfo(
		analysisInfo: AnalysisInfosForCommit | undefined,
	): void {
		...
		if (analysisInfo) {
			if (
				JSON.stringify(store.analysisInfo?.ids) !==
				JSON.stringify(analysisInfo.ids)
			) {
				// Lazy array comparison technique.
	...
	async function handleSingleLog(
		logText: string,
		outputChannel: OutputChannel,
	) {
		try {
	...
	function setBannerResultsUpdated(
		analysisInfo: AnalysisInfosForCommit | undefined,
		verb: "updated" | "unchanged" = "updated",
	) {
		if (!analysisInfo) return;

		const messageWarnStale =
			analysisInfo.commit_sha !== store.commitHash
				? ` The most recent scan was ${analysisInfo.commitsAgo} commit(s) ago` +
					` on ${new Date(
						analysisInfo.created_at,
					).toLocaleString()}.` +
					` Refresh to check for more current results.`
				: "";
		store.banner =
			`Results ${verb} for current commit ${store.commitHash.slice(
				0,
				7,
			)}.` + messageWarnStale;
	}
