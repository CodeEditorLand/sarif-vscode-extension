...
import { type IArraySplice, autorun, observable, observe } from "mobx";
...
export class Panel {
	...
	constructor(
		...
		autorun(() => {
			...
			this.panel.title = `${count} ${this.title}${
				count === 1 ? "" : "s"
			}`;
		});
		autorun(() => {
	...
	public async show() {
		...
		const { webview } = (this.panel = window.createWebviewPanel(
			"sarif",
			`${this.title}s`,
			{ preserveFocus: true, viewColumn: ViewColumn.Two }, // ViewColumn.Besides steals focus regardless of preserveFocus.
			{
		...
		webview.html = `<!DOCTYPE html>
            ...
            <head>
                ...
                <meta http-equiv="Content-Security-Policy" content="
                    ...
                    ">
                <meta name="storeState"        content='${JSON.stringify(
					Store.globalState.get("view", defaultState),
				)}'>
                <meta name="storeWorkspaceUri" content="${
					workspace.workspaceFolders?.[0]?.uri.toString() ?? ""
				}">
                <meta name="storeBanner"       content='${store.banner}'>
                <style>
                    code { font-family: ${workspace
					...
					case "selectLog": {
						...
						if (!log._jsonMap) {
							const file = fs
								...
								.replace(/^\uFEFF/, ""); // Trim BOM.
							log._jsonMap = (
								jsonMap.parse(file) as { pointers: JsonMap }
							).pointers;
						}
	...
	public async selectLocal(
		...
		editor.selection = driftedRegionToSelection(
			diffBlocks,
			currentDoc,
			region,
			originalDoc,
	...
	private createSpliceLogsMessage(removed: Log[], added: Log[]) {
		return {
			...
			added: added.map((log) => ({
				...
				webviewUri: log._text
					? ""
					: this.panel?.webview
							.asWebviewUri(
								Uri.parse(log._uriUpgraded ?? log._uri, true),
							)
							.toString(),
			})),
		};
	...
	private async spliceLogs(removed: Log[], added: Log[]) {
		await this.panel?.webview.postMessage(
			this.createSpliceLogsMessage(removed, added),
		);
	}
