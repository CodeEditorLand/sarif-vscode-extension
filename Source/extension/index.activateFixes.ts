...
import type { Fix, Result } from "sarif";
import {
	CodeAction,
	CodeActionKind,
	type Diagnostic,
...
export function activateFixes(
	disposables: Disposable[],
	store: Pick<Store, "analysisInfo" | "resultsFixed">,
	baser: UriRebaser,
) {
	disposables.push(
			...
			"*",
			{
				provideCodeActions(_doc, _range, context) {
					...
					return [
						...
						...(result.fixes?.map(
							(fix) =>
								new ResultQuickFix(diagnostic, result, fix),
						) ?? []),
						...(result.properties?.["github/alertNumber"] ===
						undefined
							? []
							: [
									// Assumes only GitHub will use `github/alertNumber`.
									new DismissCodeAction(
										diagnostic,
										result,
