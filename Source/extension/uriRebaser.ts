...
import type { VersionControlDetails } from "sarif";
...
async function workspaceHasDistinctFilename(
	filename: string,
	...
	const matches: Uri[] =
		platform === "win32"
			? (await workspace.findFiles("**/*", undefined, 5000)).filter(
					(file) =>
						path.basename(file.toString().toLowerCase()) ===
						filename,
				)
...
export class UriRebaser {
	constructor(private readonly store: Pick<Store, "distinctArtifactNames">) {}
	...
	private updateBases(artifact: string, local: Uri) {
		...
		while (
			...
			artifact[artifact.length - commonLength - 1] ===
				localPath[localPath.length - commonLength - 1]
		) {
			commonLength++;
		}
	...
	public async translateLocalToArtifact(
		localUri: Uri,
	...
	public async translateArtifactToLocal(
		...
		if (artifactUri.startsWith("sarif://"))
			return Uri.parse(artifactUri, true);
		...
		if (!validatedUri && !this.activeInfoMessages.has(artifactUri)) {
			...
			if (versionControlProvenance !== undefined) {
				const url = new URL(
					`${versionControlProvenance[0].repositoryUri}/${
						versionControlProvenance[0].revisionId
					}/${
						artifactUri.startsWith("file://")
							? artifactUri.substring(7)
							: artifactUri
					}`,
				);
				...
				if (await uriExists(fileUrl)) return fileUrl;
				...
				if (url.protocol === "https:") {
					...
					if (choice === alwaysMsg) {
						...
						workspace
							.getConfiguration(this.extensionName)
							.update(
								this.trustedSourceSitesConfigSection,
								this.trustedSites,
