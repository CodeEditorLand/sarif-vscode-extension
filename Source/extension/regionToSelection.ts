...
import type { TextDocumentLike } from "./stringTextDocument";
...
function regionToSelection(doc: TextDocumentLike, region: Region | undefined) {
	...
	if (startLine !== undefined) {
		...
		const minusOne = (n: number | undefined) =>
			n === undefined ? undefined : n - 1;

		return new Selection(
			startLine - 1,
			Math.max(
				line.firstNonWhitespaceCharacterIndex,
				minusOne(region.startColumn) ?? 0,
...
export function driftedRegionToSelection(
	diffBlocks: Change[],
	currentDoc: TextDocumentLike,
	region: Region | undefined,
	originalDoc?: TextDocumentLike,
) {
	...
	return drift === undefined
		...
		: new Selection(
				currentDoc.positionAt(
					originalDoc.offsetAt(originalRange.start) + drift,
				),
				currentDoc.positionAt(
					originalDoc.offsetAt(originalRange.end) + drift,
				),
			);
}
