...
export class ResultTableStore<G> extends TableStore<Result, G> {
	...
	private columnsPermanent = [
		new Column<Result>(
			"Line",
			50,
			(result) => result._region?.startLine?.toString() ?? "—",
			(result) => result._region?.startLine ?? 0,
		),
	...
	private columnsOptional = [
		new Column<Result>(
			"Baseline",
			100,
			(result) => result.baselineState ?? "",
		),
		new Column<Result>(
			"Suppression",
			100,
			(result) => result._suppression ?? "",
		),
		new Column<Result>(
			"Rule",
			220,
	...
	@computed get visibleColumns() {
		...
		return [
			...
			...this.columnsOptional.filter((col) =>
				optionalColumnNames.includes(col.name),
			),
		];
	}
	...
	public isLineThrough(result: Result): boolean {
		return this.resultsSource.resultsFixed.includes(
	...
	public menuContext(result: Result): Record<string, string> | undefined {
		...
		return {
			...
			resultId: JSON.stringify(result._id),
		};
	}
}
