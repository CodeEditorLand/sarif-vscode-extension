...
export function augmentLog(
	...
	log.runs?.forEach((run, runIndex) => {
		...
		run.results?.forEach((result, resultIndex) => {
			...
			result._rule =
				toolComponent?.rules?.[
					result?.rule?.index ?? result?.ruleIndex ?? -1
				] ??
				toolComponent?.rules?.find(
...
export function effectiveLevel(result: Result): Result.level {
	switch (result.kind) {
		...
		default:
			return (
				result.level ??
				result._rule?.defaultConfiguration?.level ??
				"warning"
			);
	}
}
...
export function parseArtifactLocation(
	...
	const uriContents =
		runArtCon?.text || runArtCon?.binary
			? encodeURI(
					`sarif:${encodeURIComponent(result._log._uri)}/${
						result._run._index
					}/${anyArtLoc.index}/${uri?.file ?? "Untitled"}`,
				)
			: undefined;
...
export type CommandPanelToExtension =
	| "load"
	| "open"
	| "closeLog"
	| "closeAllLogs"
